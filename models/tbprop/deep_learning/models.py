...
from tbprop.deep_learning.modules import (
    MultiLayerPerceptron,
    Conv1dBlock,
    LstmHiddenTensorExtractor,
    construct_fc_layers,
)
...
class LightningBinaryClassifier(L.LightningModule):
    def __init__(self, config):
        ...
        self.loss_fn = torch.nn.BCEWithLogitsLoss(pos_weight=torch.FloatTensor([self.pos_weight]))
        ...
        self.objective_mapping = {"f1_score": f1_score, "accuracy": accuracy_score}
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "ptl/train_loss", train_loss, on_step=True, on_epoch=True, prog_bar=True, logger=True
        )
        return train_loss
...
class FullyConnectedBinaryClassifier(LightningBinaryClassifier):
    def __init__(self, config):
        ...
        self.example_input_array = torch.randint(
            low=0, high=self.vocab_size, size=(1, self.seq_len)
        )

        self.embedding = torch.nn.Embedding(
            self.vocab_size, self.embed_size, padding_idx=self.pad_index
        )
        self.feature_extractor = torch.nn.Sequential(OrderedDict([("flatten", torch.nn.Flatten())]))

        self.fc_start_dim = self.feature_extractor(
            torch.zeros(1, self.seq_len, self.embed_size)
        ).shape[1]

        self.fc_layer_sizes = construct_fc_layers(
            self.fc_start_dim, self.fc_num_layers, self.fc_end_dim
        )
...
class ChemBERTaBinaryClassifier(LightningBinaryClassifier):
    def __init__(self, config):
        ...
        self.fc_layers = construct_fc_layers(
            start_size=self.start_size, end_size=self.end_size, num_layers=self.n_layers
        )
...
class MFBERTBinaryClassifier(LightningBinaryClassifier):
    def __init__(self, config):
        ...
        # Model components
        self.feature_extractor = MFBERT(
            weights_dir="MFBERT/Model/pre-trained", return_attention=False, inference_method="mean"
        )
        self.mlp = MultiLayerPerceptron(fc_layers=self.fc_layers)
