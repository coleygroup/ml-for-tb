...
class OptunaOptimizerForPyTorchLightning:
    def __init__(self, args, trn_dataloader, val_dataloader, model_name="", dataset="",
                 eval_metric='ptl/val_loss', n_warmup_steps=5, seq_len=512, 
                 early_stopping_patience=7, verbose=0):
        self.args = args
    ...
    def objective(self, trial):
        ...
        callbacks = [
            EarlyStopping(monitor=self.eval_metric, mode="min", 
                          patience=self.early_stopping_patience), 
            PyTorchLightningPruningCallback(trial, monitor=self.eval_metric)
        ]

        trainer = Trainer(max_epochs=self.args.n_epochs,
