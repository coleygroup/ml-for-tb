...
class SmilesDataset(Dataset):
    ...
    def __init__(self, file_path, tokenizer, model_name, 
                 target_col='AUC_bin', smiles_col='mol', tokenizer_max_length=512):
        ...
        self.recognized_model_names = \
                ('FCNN', 'CNN', 'LSTM', 'SmilesTransformer', 
                 'MFBERT', 'ChemBERTa-v2')
        ...
        if self.model_name in ('FCNN', 'CNN', 'LSTM', 'SmilesTransformer', 'ChemBERTa-v2'):
            self.encodings = tokenizer(self.X, 
                                       padding='max_length', 
                                       truncation=True,
...
class SmilesDataModule:
    def __init__(self, dataset: str, data_dir: str, tokenizer, model_name: str, target_col: str,
                 smiles_col: str, train_batch_size: int = 512, infer_batch_size: int = 512, 
                 num_workers: int = 7, tokenizer_max_length: int = 512):
        super().__init__()
        ...
        self.trn_set = SmilesDataset(f"{self.dataset_prefix}_trn.csv", 
                                        self.tokenizer, 
                                        model_name=self.model_name, 
                                        target_col=self.target_col, 
                                        smiles_col=self.smiles_col, 
                                        tokenizer_max_length=self.tokenizer_max_length)
        self.val_set = SmilesDataset(f"{self.dataset_prefix}_val.csv", 
                                        self.tokenizer, 
                                        model_name=self.model_name, 
                                        target_col=self.target_col, 
                                        smiles_col=self.smiles_col, 
                                        tokenizer_max_length=self.tokenizer_max_length)
        self.tst_set = SmilesDataset(f"{self.dataset_prefix}_tst.csv", 
                                        self.tokenizer, 
                                        model_name=self.model_name, 
                                        target_col=self.target_col, 
                                        smiles_col=self.smiles_col, 
        ...
        self.trn_dataloader = DataLoader(self.trn_set, 
                                         batch_size=self.train_batch_size, 
                                         num_workers=self.num_workers, 
                                         persistent_workers=True, 
                                         shuffle=True)
        
        self.trn_val_dataloader = DataLoader(self.trn_val_set,
                                             batch_size=self.train_batch_size,
                                             num_workers=self.num_workers,
                                             persistent_workers=True,
                                             shuffle=True)
        ...
        self.tst_dataloader = DataLoader(self.tst_set,
                                         batch_size=self.infer_batch_size,
                                         num_workers=self.num_workers,
                                         persistent_workers=True,
                                         shuffle=False)
        
        self.full_dataloader = DataLoader(self.full_set,
                                          batch_size=self.infer_batch_size,
                                          num_workers=self.num_workers,
                                          persistent_workers=True,
                                          shuffle=False)
