...
from tbprop.tree_based.preprocessing import (
    calc_high_corr_feats,
    calc_low_variance_feats,
    scale_columns,
)
...
def resample_data(
    ...
    if undersampler is not None:
        if verbose:
            print(f"Undersampling with {undersampler}...")
        us_dict = {
            "TomekLinks": TomekLinks(),
            "ClusterCentroids": ClusterCentroids(),
            "RandomUnderSampler": RandomUnderSampler(
                sampling_strategy=undersampling_ratio, random_state=random_state
            ),
        }
        ...
        if verbose:
            print("No undersampler.")

    if oversampler is not None:
        if verbose:
            print(f"Oversampling with {oversampler}...")
        os_dict = {
            "ADASYN": ADASYN(sampling_strategy=oversampling_ratio, random_state=random_state),
            "SMOTE": SMOTE(sampling_strategy=oversampling_ratio, random_state=random_state),
            "RandomOverSampler": RandomOverSampler(
                sampling_strategy=oversampling_ratio, random_state=random_state
            ),
        }
...
class FeatureEngineeringPipeline:
    ...
    def fit_transform(self, df_train, df_test, remove_cols, ycol="AUC_bin"):
        ...
        high_corr_feats = calc_high_corr_feats(df_trn, remove_cols, threshold=self.corr_thresh)
        remove_cols.update(high_corr_feats)

        # Remove low variance features
        low_var_feats = calc_low_variance_feats(df_trn, remove_cols, threshold=self.var_thresh)
        remove_cols.update(low_var_feats)

        # Scale columns
        df_train_scaled, df_test_scaled, scaler = scale_columns(df_trn, df_tst, list(remove_cols))

        # Add morgan fingerprints
        df_train_scaled = morgan_fingerprint(
            df_train_scaled, smiles_col=self.smiles_col, n_bits=self.fp_n_bits
        )
        df_test_scaled = morgan_fingerprint(
            df_test_scaled, smiles_col=self.smiles_col, n_bits=self.fp_n_bits
        )
        ...
        # Resample data
        X_train, y_train, us, os = resample_data(
            X_train,
            y_train,
    ...
    def transform(self, df_infer):
        ...
        if self.final_col_set - set(df.columns):
            raise ValueError(
                f"Inference set doesn't have the following columns: {self.final_col_set - set(df_infer.columns)}."
            )
