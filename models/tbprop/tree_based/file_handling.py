...
def read_pk_dataset_with_split(train_path, test_path, val_path=None, mode='bin_class', ycol='AUC', verbose=True):
    """
    ...
    if verbose:
        ...
        print(f"Columns with null values = {list(df_train.isnull().sum()[df_train.isnull().sum() > 0].keys())}")

        print(f"Shape of test data = {df_test.shape}")
        print(f"Columns with null values = {list(df_test.isnull().sum()[df_test.isnull().sum() > 0].keys())}")
    ...
    if val_path:
        ...
        if verbose:
            ...
            print(f"Columns with null values = {list(df_val.isnull().sum()[df_val.isnull().sum() > 0].keys())}")
    else:
        df_val = None
