...
def compare_two_models(X, y, est1, est2, preds1=None, preds2=None, test_type="mcnemar"):
    ...
    if test_type == "mcnemar":
        # metric = chi2
        metric, p = mcnemar(mcnemar_table(y, preds1, preds2), corrected=False)
    elif test_type == "5x2cv":
        # metric = f
        metric, p = combined_ftest_5x2cv(estimator1=est1, estimator2=est2, X=X, y=y)
    else:
...
def hypothesis_test_multiple_comparison(
    estimators,
    predictions,
    X_test,
    y_test,
    ...
    clf_combinations = list(itertools.combinations(range(len(estimators)), 2))
    ...
    for i, j in clf_combinations:
        metric, p_value = compare_two_models(
            X_test,
            y_test,
            est1=clf_objs[i],
            est2=clf_objs[j],
        ...
        if not post_hoc_alpha / 2 <= p_value <= 1 - (post_hoc_alpha / 2):
            print(
                f"Combination: {(clf_names[i], clf_names[j])},\n\t p-value = {round(p_value, 3)}\n"
            )
...
def compare_models_optimizers_on_split(
    models,
    X_trn,
    y_trn,
    X_tst,
    ...
    for optimizer in optimizer_types:
        ...
        opt_trained_models, opt_test_metrics = optimize_models(
            models,
            optimizer,
            X_trn,
            y_trn,
    ...
    test_metrics = pd.DataFrame(
        np.concatenate(test_metrics, axis=0), columns=["model", metric, "lb", "ub", "thresh_values"]
