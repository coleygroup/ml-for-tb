...
def setup():
    ...
    exp_group.add_argument("--data_dir", type=str, default="./data", 
                           help="Location where all data is saved.")
    exp_group.add_argument("--dataset", type=str, default="taacf", 
                           choices=["pk", "taacf", "mlsmr"], help="Name of dataset.")
    ...
    exp_group.add_argument("--find_threshold", type=str, default="f1", choices=["none", "f1", "j"],
                           help="Finds threshold that maximizes the given metric on val set.")
    exp_group.add_argument("--tb_inc_timestamp", action="store_true", 
                           help="Whether to include timestamp in tensorboard study name.")
    exp_group.add_argument("--use_best_config", action="store_true", 
                           help="Uses best config saved at data_dir/configs")
    exp_group.add_argument("--use_saved_model", action="store_true", 
    ...
    model_group.add_argument("--model_name", type=str, default="FCNN", 
                             choices=['FCNN', 'CNN', 'LSTM', 'ChemBERTa-v2'], help="Name of model.")
    model_group.add_argument("--n_devices", type=int, default=1, 
                             help="n_devices to train model on.")
    model_group.add_argument("--num_workers", type=int, default=7, 
                             help="num_workers for torch dataloaders.")
    model_group.add_argument("--patience", type=int, default=10, 
                             help="No. of epochs to wait for early stopping.")
    ...
    model_group.add_argument("--device", type=str, default="cpu", 
                             choices=["cpu", "mps", "gpu"], help="Device to train on.")
    ...
    model_group.add_argument("--eval_metric", type=str, default="ptl/train_loss_epoch", 
...
def main():
    ...
    if args.verbose: print(f"Loading data...")
    dm = SmilesDataModule(dataset=args.dataset, 
    ...
    if args.find_threshold != "none" and not (f"best_threshold_{args.find_threshold}" in config):
        ...
        if args.find_threshold == "f1":
            ...
            raise NotImplementedError(f"Threshold determination using Youden's J not implemented yet.")

        trainer = Trainer(max_epochs=args.n_epochs,
                          accelerator=args.device,
                          devices=args.n_devices,
                          log_every_n_steps=1,
                          callbacks=[EarlyStopping(monitor=args.eval_metric, 
        ...
        t_metrics = \
            [evaluator_fn(y_val, (val_preds > t).astype(int)) for t in np.arange(0., 1.01, 0.02)]
        ...
        if args.verbose:
            print(f"Found best threshold {round(best_threshold, 2)}. Saving to best config path if exists.")
    ...
    trainer = Trainer(max_epochs=args.n_epochs,
                      ...
                      callbacks=[EarlyStopping(monitor=args.eval_metric, 
                                               patience=args.patience,
                                               mode="min"),
                                 ModelCheckpoint(dirpath=f"{args.data_dir}/checkpoints",
                                                 filename=ckpt_fname,
    ...
    if args.verbose: print(f"Predicting and saving to disk...")
    ...
    with open(f"{args.data_dir}/predictions/{args.dataset}_{args.model_name}_tst_probs.npy", "wb") as f:
        np.save(f, test_probs)
    with open(f"{args.data_dir}/predictions/{args.dataset}_{args.model_name}_tst_preds.npy", "wb") as f:
