...
from pk.datasets import PKSmilesDataset
...
def finetune(model, train_dataloader, n_epochs=5,
    ...
    # For logging
    train_losses = []
    epoch_iterator = tqdm(range(n_epochs)) if not verbose else range(n_epochs)
    
    # Train
    for epoch in epoch_iterator:
        if verbose: print(f"Epoch # {str(epoch+1).zfill(3)}: ", end="")
        itr_losses, t0 = [], time()
        
        # Iterate over batches
        for batch_ndx, (X_batch, y_batch) in enumerate(train_dataloader):
            if verbose: print(f"Itr {batch_ndx+1}...", end="")
...
def inference(model, test_dataloader, pred_threshold=0.5):
    """ 
    ...
    for batch_ndx, (X_batch, y_batch) in enumerate(test_dataloader):
        scores = model(X_batch).reshape(-1)
    ...
    accumulate = lambda x: torch.cat(x).detach().numpy()
...
def evaluate_model(args):
    ...
    train_set = PKSmilesDataset(args.train_path, tokenizer)
    test_set = PKSmilesDataset(args.test_path, tokenizer)
...
def embeddings(model, dl, to_dataframe=False, **kwargs):
    ...
    for _, (X_batch, y_batch) in enumerate(dl):
        batches.append(model.embed(X_batch))
        labels.append(y_batch)
    X, y = torch.cat(batches, 0).detach().numpy(), torch.cat(labels, 0).detach().numpy()
