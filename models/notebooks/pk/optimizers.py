...
from pk.search_spaces import optuna_space, hyperopt_space, \
                             random_search_space
from pk.model_selection import CrossValidator, randomized_search
from pk.metrics import binary_classification_metrics, regression_metrics
from pk.utils import adjust_params_by_model


SUPPORTED_CLF_METRICS = ['n_pos', 'accuracy', 'balanced_accuracy', 
                         'auroc', 'precision', 'recall', 'f1_score']
...
class Optimizer:
    ...
    def __init__(self, X_train, y_train, estimator, metric='auroc', 
                 X_val=None, y_val=None, val_mode='k_fold_fixed_split', random_state=1):
        """
        ...
        self.supported_metrics = \
                self.supported_clf_metrics + self.supported_reg_metrics
        ...
        if self.val_mode.startswith('k_fold') and self.X_val is not None and self.y_val is not None:
            ...
            if isinstance(self.y_train, pd.Series):
                ...
                raise ValueError(
                    "y_train must be either a pd.Series or a np.ndarray."
                )
    ...
    def _objective_with_params(self, params):
        ...
        if self.val_mode == 'k_fold_random_split':
            ...
            score = -np.where(self.metric_names == self.metric, mean_metrics, 0.).sum()/100
            ...
            if self.mode == 'classification':
                ...
                metrics = binary_classification_metrics(self.y_val, y_pred, y_score, return_type='dict')
            else:
    ...
    def plot_metrics(self):
        ...
        plt.title(title)
        plt.plot(range(len(self.metric_by_iter)), self.metric_by_iter, 
                label=f"{self.metric}")
...
class HyperoptOptimizer(Optimizer):
    ...
    def __init__(self, X_train, y_train, estimator, metric='auroc', X_val=None, y_val=None, val_mode='k_fold_random_split', random_state=1):
...
def optimize_models(models, optimizer, X_trn, y_trn, X_tst, y_tst,
                    X_val=None, y_val=None, random_state=1, random_seed=1, metric='auroc', 
                    max_evals=20, val_mode='k_fold_random_split', verbose=True, pipeline_suffix=''):
    ...
    for model in models:
        ...
        if optimizer == 'hyperopt':
            opt = HyperoptOptimizer(X_trn, y_trn, model, 
            ...
            best_params = fmin(opt.objective,
            ...
            opt = OptunaOptimizer(X_trn, y_trn, model, 
                                  X_val=X_val, y_val=y_val, 
                                  val_mode=val_mode,
                                  metric=metric, 
                                  random_state=random_state)
            ...
            if val_mode.startswith('k_fold'):
                results = randomized_search(
                    X_trn, y_trn, X_tst, y_tst, est, params=random_search_space(model), verbose=verbose
                )
            else:
                results = randomized_search(
                    pd.concat([X_trn, X_val]), np.concatenate([y_trn, y_val]), 
                    X_tst, y_tst, est, params=random_search_space(model), verbose=verbose
                )
    ...
    test_metrics = [['model', metric, 'lb', 'ub']]

    for i, model in enumerate(models):
        model_name, best_model, _ = adjust_params_by_model(best_params_collect[i], 
                                                           model, 
                                                           optimizer=optimizer,
                                                           init=True)
        if verbose:
        ...
        test_metrics.append([model_name + '/' + optimizer + '/' + pipeline_suffix, metric_val, 0., 100.])

        if verbose:
            # print(f"Test AUC: {auc} +/- ({lb}, {ub})")
            print(f"Test {metric}: {metric_val}")
    ...
    for i, model in enumerate(models):
        ...
        cv = CrossValidator(mode='reg' if metric in SUPPORTED_REG_METRICS else 'bin_class', 
                            estimator=model, 
