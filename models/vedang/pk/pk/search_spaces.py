...
def hyperopt_space(estimator):
    ...
    if estimator_type in ('RandomForestClassifier', 'RandomForestRegressor'):
        params = {
            'n_estimators': hp.uniformint('n_estimators', 50, 1000+1),
            'max_depth': hp.uniformint('max_depth', 1, 100+1),
            'min_samples_split': hp.uniformint('min_samples_split', 2, 100+1),
            'n_jobs': -1
        }
    elif estimator_type in ('XGBClassifier', 'XGBRegressor'):
        params = {
            'max_depth': hp.uniformint('max_depth', 2, 10+1),
            ...
            'scale_pos_weight': hp.uniformint('scale_pos_weight', 1, 100+1),
            'nthread' : -1
        }
    elif estimator_type in ('LGBMClassifier', 'LGBMRegressor'):
        params = {
            'n_estimators': hp.uniformint('n_estimators', 20, 100+1),
            'learning_rate': hp.loguniform('learning_rate', np.log(0.005), np.log(0.2)),
            'max_depth': hp.uniformint('max_depth', 3, 12+1), 
            'num_leaves': hp.uniformint('num_leaves', 20, 100+1),
            'verbose': -1, 
            'n_jobs': -1
        }
    elif estimator_type in ('CatBoostClassifier', 'CatBoostRegressor'):
        params = {
            'iterations': hp.uniformint('iterations', 100, 500+1),
            'depth': hp.uniformint('depth', 1, 9+1),
            'learning_rate': hp.uniform('learning_rate', 0.001, 0.1),
            'l2_leaf_reg': hp.uniform('l2_leaf_reg', 0.001, 0.1),
            'border_count': hp.uniformint('border_count', 32, 255+1),
